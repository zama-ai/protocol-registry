...
Each file is a stable, sorted-key JSON object: `{ schema_version, chains, contracts, lz_dvn_configs, parameters }`. Contract entries carry `address`, `chain`, `type`, and (where applicable) `owner`/`owner_address`, `symbol`, `note`. Operator staking and rewarder contracts live in `contracts` (types `operator_staking` / `operator_rewarder`), each carrying its own `chain`, `operator`, and `role`.
